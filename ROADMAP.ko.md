...
## 1.6.0까지 구현 완료 (Shipped Through 1.6.0)

`1.6.0`(이번 릴리스)은 에이전트 네이티브 라인이다: `llm-wiki mcp`가 stdio 위에서 Model
...
`GATE_REVIEW.md` 게이트(다음은 Gate 12)로 기록한다 — 이전의 모든 범위 결정을 규율한
바로 그 방식이다.
...
원래 번들에서 **다른 넷에 대한 의존성이 없고, 불변식을 지는 코어 스캐너를 건드리지
않는** 유일한 기능 — 따라서 가장 저위험·고레버리지이며, 조기 출시가 나머지 마이너의
설계가 굳기 전에 필요한 다중 팀 피드백을 만든다.

- **1급 GitHub Action** — 읽기 전용 `validate`를 `npx`로 감싸는 한 `uses:` 스텝짜리
  composite action.
- **태그 push 시 GitHub Release** — 릴리스 노트로 생성하며
  `.github/workflows/publish.yml`을 확장한다. 러너의 `gh` CLI로 만들고(서드파티 액션
  없음 — 무의존성 원칙 보호), `contents: write`로 격리된 잡에서, 릴리스 본문은
  민감정보 스캔을 거친다.
- **`help`의 명령별 `--format json` 예시**(백로그에서 승격) — Action / 래퍼 / MCP
  작성자용.

전용 게이트로 보류: Marketplace 게시와 Action floating-tag 버저닝. 먼저 `v*`
npm-publish 태그 네임스페이스와 `publish.yml`의 버전 일치 가드를 deconflict해야 한다.
...
위 릴리스 계획으로 승격: 명령별 JSON `help` 예시(→ 1.7), 더 풍부한 enrichment 린팅
(→ 1.8, 토글 가능한 `content.thin_body` 규칙으로).
