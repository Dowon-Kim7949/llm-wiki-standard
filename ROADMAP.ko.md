...
**1.8.0 출시:** per-project rule 토글(`rules` 맵)과 `content.thin_body` opt-in lint;
severity 수렴 pre-work 완료(감사로 동작 보존 확인, 불일치 0). `1.8.x` 잔여: 커스텀 문서셋과
템플릿 오버라이드(never-`verified` 가드레일 포함).
